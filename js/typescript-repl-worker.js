...
async function loadCompilerOptions() {
    ...
    try {
        const response = await fetch('tsconfig.json', { cache: 'no-cache' });
        if (!response.ok) {
            compilerOptions = defaults;
            return;
        }
        const config = await response.json();
        if (!config || typeof config !== 'object' || !config.compilerOptions) {
            compilerOptions = defaults;
            return;
        }
        if (typeof tsCompiler.convertCompilerOptionsFromJson === 'function') {
            const converted = tsCompiler.convertCompilerOptionsFromJson(config.compilerOptions, '');
            compilerOptions = {
                ...defaults,
                ...(converted && converted.options ? converted.options : {})
            };
        } else {
            compilerOptions = {
                ...defaults,
