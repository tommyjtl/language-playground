/**
 * TypeScript Output - Runs full scripts via Web Worker and displays output
 */
...
const TypeScriptOutput = {
    ...
    init(options = {}) {
        ...
        DebugLogger.updateTsOutputWorkerStatus('Creating worker...');
        return new Promise((resolve, reject) => {
            this.worker = new Worker(new URL('./typescript-output-worker.js', import.meta.url));

            this.worker.onmessage = (event) => {
                const { type, message, exception, exitCode, stream, text } = event.data;
                ...
                switch (type) {
                    case 'status':
                        ...
                        DebugLogger.updateTsOutputWorkerStatus(message);
                        break;
                    ...
                    case 'ready':
                        ...
                        DebugLogger.updateTsOutputWorkerStatus('Ready');
                        resolve();
                        break;
                    ...
                    case 'error':
                        ...
                        DebugLogger.updateTsOutputWorkerStatus(`Error: ${message}`);
                        break;
            ...
            this.worker.onerror = (error) => {
                ...
                DebugLogger.updateTsOutputWorkerStatus(`Error: ${error.message}`);
                reject(error);
            };
    ...
    appendStream(text, type) {
        ...
        if (this.debugStream) {
            DebugLogger.log(`[TS Output:${type}] ${JSON.stringify(normalized)}`);
        }
        this.outputEl.scrollTop = this.outputEl.scrollHeight;
    },
    ...
    terminate() {
        if (this.worker) {
            ...
            this.worker = null;
            DebugLogger.updateTsOutputWorkerStatus('Terminated');
        }
        if (this.isRunning) {
            const terminationError = new Error('TypeScript run terminated');
            if (this._runReject) {
